...
v2.1.0 — 內網 P2P 多人協作工具 (WebRTC + WebSocket)。即時聊天、檔案傳輸、公告欄、檢查清單、密鑰管理。附 JS SDK (clipper-sdk.js)、REST API、WebSocket 協定文件。離線唯讀防護、幽靈復活防護。適用於廣播電視台內網協作。
...
### 一鍵啟動（PyInstaller 打包版）

```bash
# 下載 Release 中的執行檔
# Windows: clipper-server.exe（雙擊執行）
# macOS: clipper-server（終端執行）或 Clipper.app（雙擊）

# 啟動後自動提供：
# - WebSocket: ws://localhost:8765
# - HTTP: http://localhost:8766（瀏覽器開啟）
# - 預設配對碼: 1234
```
...
[![Build Status](https://github.com/hug0-l/Clipper/actions/workflows/build.yml/badge.svg)](https://github.com/hug0-l/Clipper/actions/workflows/build.yml)

| 平台 | 產出 | 觸發方式 |
|------|------|---------|
| 🪟 Windows | `clipper-server.exe` | `git tag v2.1.0 &amp;&amp; git push origin v2.1.0` |
| 🍎 macOS | `clipper-server` + `Clipper.app.zip` | 同上 |
...
├── signal_server.py      # 信令伺服器（WebSocket 配對 + 持久化儲存 + Relay，~1,200 行）
├── AGENTS.md             # AI 輔助開發指引
├── CHANGELOG.md          # 版本變更記錄
├── README.md             # 本文件
...
DB_PATH = "clipper_data.db"  # SQLite 資料庫路徑
LOG_DIR = "logs"            # 日誌檔案目錄
...
| 限制 | 建議解法 |
|------|---------|
| ~~需要 Python 3.8+~~ | ✅ **PyInstaller 已打包** — 下載 Release 中的執行檔即可運作，無需安裝 Python |
...
| Windows 雙擊無反應 | 從終端執行確認錯誤訊息，或檢查 Windows Defender 是否攔截 |
